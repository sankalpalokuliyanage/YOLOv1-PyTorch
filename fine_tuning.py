...
# Phase 1 model
LOAD_MODEL_FILE = "final_yolo_model.pth"
...
# --- Advanced Transforms for Fine-Tuning ---
...
def main():
    print(f"Initializing model for Fine-Tuning (Device: {DEVICE})...")
    ...
    if os.path.exists(LOAD_MODEL_FILE):
        print(f"==> Loading previous knowledge from: {LOAD_MODEL_FILE}")
        checkpoint = torch.load(LOAD_MODEL_FILE, map_location=torch.device(DEVICE))
        model.load_state_dict(checkpoint['model_state_dict'])
        print("Checkpoint loaded! Ready to refine features.")
    else:
        print("Error: Starting file not found. Run train.py first.")
        return

    train_dataset = CustomYOLODataset(
        img_dir="data/images", label_dir="data/labels", transform=fine_tune_transform, C=C
    )
    ...
    print(f"Starting Fine-Tuning for {EPOCHS} epochs (LR: {LEARNING_RATE})")
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 1) % 10 == 0:
            torch.save({
                ...
                'epoch': epoch + 1,
            }, "fine_tuned_yolo_model.pth")
            print("==> Fine-tuned model updated: fine_tuned_yolo_model.pth")
